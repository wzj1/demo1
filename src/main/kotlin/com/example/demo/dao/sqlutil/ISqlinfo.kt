...
abstract class ISqlinfo {
    ...
    // 查询表
    val selectLogin = "$select $login_Table "
    val selectUser = "$select $user_table "
    val selectUserInfo = "$select $userinfo_table "
    //查询表中的数据

    val selectLoginCount = "$select count(*) $login_Table "
    val selectUserCount = "$select count(*) $user_table "
    val selectUserInfoCount = "$select count(*) $userinfo_table "
     ...
     * @param sql
     */
    fun isSqlLog(sql: String): String {
        println("sql:$sql")
        return sql
    }
